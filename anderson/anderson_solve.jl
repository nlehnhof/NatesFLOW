...
function get_kernel(panels::AbstractPanels{K,<:Any,<:Any,<:Any}) where K
    return K()
end
...
function update_influence_matrix!(influence_matrix,
                                    panels::Tuple,
                                    ::Type{Scheme{DirectNeumann, FlowTangency}};
                                    panel_indices = Tuple((1:length(p.panels) for p in panels)),
                                    set_unit_strength=false
                                    )
    ...
    if set_unit_strength
        for p in panels
            panel_index = 1:length(p.panels)
            panels_2_grid_strength!(p; panel_indices=panel_index)
            set_unit_strength!(p; panel_indices=panel_index)
        end
    end
    ...
    # Uinfmag = 1.0
    # alpha = 15 * pi/180
    # Uinf = SVector{3}(cos(alpha), sin(alpha), 0.0) * Uinfmag

    # update influence matrix
    i_row_offset = 0
    for target_body in panels
        n_target = length(target_body.panels)
        i_col_offset = 0
        for source_body in panels
            n_source = length(source_body.panels)
            kernel = get_kernel(source_body)
            for (j, source_panel) in enumerate(source_body.panels)
                for (k, target_panel) in enumerate(target_body.panels)
                    _, v, _ = induced(target_panel.control_point, source_panel, kernel,
                                    DerivativesSwitch(false, true, false))
                    influence_matrix[i_row_offset + k, i_col_offset + j] = dot(v, target_panel.normal)
                    # i_row_offset + k, i_col_offset + j
                end
            end
            i_col_offset += n_source   # accumulate columns properly
        end
        i_row_offset += n_target       # accumulate rows properly
    end
    # @show panels[1].strengths[1:5]

    # restore panel strengths

    for p in panels
        panel_index = 1:length(p.panels)
        set_unit_strength && grid_2_panels_strength!(p; panel_indices=panel_index)
    end
end
...
"""
function update_right_hand_side!(right_hand_side, panels::Tuple, ::Type{Scheme{DirectNeumann, FlowTangency}}, freestream=SVector{3,Float64}(0,0,0); reset=true)

    # number of panels per body
    n_panels = 0
    for p in panels
        n_panels += length(p.panels)
    end

    panel_indices = 1:n_panels

    # reset RHS
    reset && (right_hand_side[panel_indices] .= zero(eltype(right_hand_side)))

    j = 1
    # update right hand side per body
    for (body_index, p) in enumerate(panels)
        n_panels = length(p.panels)
        # per panel in body
        for i in 1:n_panels
            panel = p.panels[i]
            velocity = p.velocity[i]
            right_hand_side[j] -= dot(freestream + velocity, panel.normal)
            j += 1
        end
    end
end
...
function get_dof(panels::AbstractPanels{K,<:Any,<:Any}) where K<:Source{1}
    return length(panels.panels)
end

function get_dof(panels::AbstractPanels{K,<:Any,<:Any}) where K<:SourceNormalDoublet{2}
    return length(panels.panels)-1
end
function get_dof(panels::AbstractPanels{K,<:Any,<:Any}) where K<:NormalDoublet{1}
    return length(panels.panels)-1
end
...
function LUDecomposition(panels::Tuple, scheme; 
                            save_lu= false,
                            sanity_check=false
                            )
    # preallocate memory
    ...
    # Set up matrices
    influence_matrix = zeros(Float64, n_panels, n_dof)
    right_hand_side = zeros(Float64, n_panels)

    # Update influence matrix
    update_influence_matrix!(influence_matrix, panels, scheme; set_unit_strength=true)
    
    # get LU Decomposition
    lu_decomposition = nothing
    save_lu && (influence_matrix = lu!(influence_matrix))

    strengths = zeros(Float64, n_dof)
...
function solve!(panels::Tuple,
    ...
    # @show normals_rhs = [p.panels[i].normal for p in panels for i in 1:length(p.panels)]
    ...
    for p in panels
        ...
        for i in 1:n_panels
            # (; vertices, control_point, normal, strength, radius) = panels.panels[i]
            vertices = p.panels[i].vertices
            control_point = p.panels[i].control_point
            normal = p.panels[i].normal
            old_strength = p.panels[i].strength
            radius = p.panels[i].radius
            strength = get_strength(strengths, j, old_strength)
            j += 1

            p.panels[i] = Panel(vertices, control_point, normal, strength, radius)
            p.strengths[i] = strength
        end
    end
end
